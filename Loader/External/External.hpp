...
WPARAM MainLoop(forceinline::dx_renderer* renderer)
{
	...
	while (Message.message != WM_QUIT)
	{
		...
		if (currHWND == sdk::game_wnd) {
			...
			if (show_menu) {
				if (menu_system.Window(skCrypt("Cheat by https://github.com/realCosmicHub"), pos, FVector2D(400, 400), show_menu)) {
					...
					//	menu_system.Checkbox(skCrypt("Crosshair"), &settings::crosshair);
						...
						switch (weapontab) {
						case 0:
							menu_system.SliderInt(skCrypt("AR Smooth"), &settings::ar_smooth, 1, 20);
							menu_system.Combobox(skCrypt("AR Aimbone"), FVector2D(150, 20), &ar_index, skCrypt("Head").decrypt(), skCrypt("Neck").decrypt(), skCrypt("Chest").decrypt(), skCrypt("Pelvis").decrypt(), NULL);//sec //test if work
							break;
						case 1:
							menu_system.SliderInt(skCrypt("Shotgun Smooth"), &settings::shotty_smooth, 1, 20);
							menu_system.Combobox(skCrypt("Shotgun Aimbone"), FVector2D(150, 20), &shotgun_index, skCrypt("Head").decrypt(), skCrypt("Neck").decrypt(), skCrypt("Chest").decrypt(), skCrypt("Pelvis").decrypt(), NULL);//sec //test if work
							break;
						case 2:
							menu_system.SliderInt(skCrypt("SMG Smooth"), &settings::smg_smooth, 1, 20);
							menu_system.Combobox(skCrypt("SMG Aimbone"), FVector2D(150, 20), &smg_index, skCrypt("Head").decrypt(), skCrypt("Neck").decrypt(), skCrypt("Chest").decrypt(), skCrypt("Pelvis").decrypt(), NULL);//sec //test if work

							break;
						case 3:
							menu_system.SliderInt(skCrypt("Other Smooth"), &settings::other_smooth, 1, 20);
							menu_system.Combobox(skCrypt("Other Aimbone"), FVector2D(150, 20), &other_index, skCrypt("Head").decrypt(), skCrypt("Neck").decrypt(), skCrypt("Chest").decrypt(), skCrypt("Pelvis").decrypt(), NULL);//sec //test if work
					...
					//	menu_system.Checkbox(skCrypt("Controller Mode"), &settings::controllersupport);
						menu_system.Checkbox(skCrypt("Player Indicator ( Buggy )"), &settings::enemyindicator);
...
void init()
{
	...
	std::cout << skCrypt("\n\n [-] Connecting to driver\n");
		...
		SetConsoleTitleA("Cheat by https://github.com/realCosmicHub");
		printf("Injected ( don't close this window :D )\n");
